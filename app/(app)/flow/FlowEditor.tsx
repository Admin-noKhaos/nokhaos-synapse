...
const PALETTE = [
  ...
  { kind: 'action', items: [
    ...
    { type: 'follow_up', label: 'Follow up later', icon: <I.Calendar size={14} />, color: '#DDA0FF' },
...
function summarize(n: FlowNode): string {
  ...
  if (n.kind === 'action') {
    ...
    if (n.type === 'follow_up') {
      const ds = (n.config.delays_hours ?? []) as number[];
      return ds.length ? `nudge at +${ds.join('h, +')}h if no reply` : 'no schedule set';
    }
...
function Properties({
  ...
  return (
    <aside style={{
      ...
      {node.kind === 'action' && node.type === 'follow_up' && (
        <>
          <Field label="Send after (hours, comma-separated)">
            <input className="sx-input" placeholder="2, 6, 12"
              value={((node.config.delays_hours ?? []) as number[]).join(', ')}
              onChange={(e) => patch({ delays_hours: e.target.value.split(',').map((s) => Number(s.trim())).filter((n) => Number.isFinite(n) && n > 0) })} />
            <div style={{ fontSize: 11, color: 'var(--text-3)', marginTop: 6, lineHeight: 1.5 }}>
              Each nudge is sent that many hours after the last message — but only if the lead still hasn’t replied. The moment they reply, the rest are cancelled.
            </div>
          </Field>
          <Field label="Follow-up goal (AI-generated)">
            <textarea className="sx-input" style={{ height: 80, paddingTop: 8 }}
              placeholder="They were sent the training link. Check in to see if they've watched it. Don't resend the link."
              value={node.config.goal ?? ''} onChange={(e) => patch({ goal: e.target.value })} />
            <div style={{ fontSize: 11, color: 'var(--text-3)', marginTop: 6, lineHeight: 1.5 }}>
              Each nudge is generated by AI from this goal + the conversation + your master doc. Leave the variations below empty to use AI.
            </div>
          </Field>
          <Field label="Or fixed message variations (skip AI)">
            <VariantsEditor
              variants={(node.config.variants ?? []) as string[]}
              onChange={(variants) => patch({ variants })}
              placeholder="Hey, did you get a chance to check it out?"
              max={5}
            />
          </Field>
        </>
